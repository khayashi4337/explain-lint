...
| ISSUE-12 | 中 | scan()がファイル不在時の例外を未ハンドリング（既存バグ） | fix/12-scan-file-not-found | open |
| ISSUE-13 | 低 | レビュー3回イテレートのスキップ（ISSUE-08〜11） | — | open |
| ISSUE-14 | 高 | get_context()がPDF入力で壊れる（ページ番号を行インデックスとして誤用） | fix/14-pdf-context-broken | open |
| ISSUE-15 | 中 | フォールバック形態素解析が極めてノイズが多い（文全体が1用語になる） | fix/15-fallback-morph-noise | open |
| ISSUE-16 | 低 | MCPサーバーに索引生成ツールがない | fix/16-mcp-index-tool | open |
