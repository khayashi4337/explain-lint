...
| ISSUE-03 | 高 | 非UTF8コンソールで出力クラッシュ（`—`/`§`） | fix/03-unicode-output | open |
| ISSUE-04 | 高 | 抽出方針のハードコーディング（AT&T漏れ・大文字限定・最小長分散） | fix/04-extraction-config | open |
| ISSUE-05 | 中 | 自動テスト不在 | fix/05-tests | open |
| ISSUE-06 | 中 | 1ファイル集約→パッケージ分割＋定数一元化 | fix/06-package-split | open |
| ISSUE-07 | 低 | preamble内の表行をデータ行と誤認（ISSUE-02レビュー由来） | fix/07-preamble-robustness | open |
