...
| ID | 深刻度 | 概要 | branch |
|---|---|---|---|
| ISSUE-01 | 高 | MOVED語がrecord_judgment後も消えない（hash非再同期） | fix/01-moved-resync |
| ISSUE-02 | 高 | セル内`\|`で台帳の行が消失（非対称エスケープ） | fix/02-ledger-pipe |
| ISSUE-03 | 高 | 非UTF8コンソールで出力クラッシュ（`—`/`§`） | fix/03-unicode-output |
| ISSUE-04 | 高 | 抽出方針のハードコーディング（AT&T漏れ・大文字限定・最小長分散） | fix/04-extraction-config |
| ISSUE-05 | 中 | 自動テスト不在 | fix/05-tests |
| ISSUE-06 | 中 | 1ファイル集約→パッケージ分割＋定数一元化 | fix/06-package-split |
