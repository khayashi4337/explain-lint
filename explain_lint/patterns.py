...
# 漢字の連続、または漢字+ひらがな（2文字以上）の複合語（形態素解析なしのフォールバック用）。
# 1文字のひらがな（助詞: と、に、は、が等）で分割されるように、
# 漢字連続 [一-龥]+ とひらがな2文字以上 [ぁ-ゟ]{2,} を別々にマッチさせる。
# 形態素解析が利用できない場合、これで粗く抽出する。
KANJI_KANA = re.compile(r"[一-龥]+(?:[ぁ-ゟ]{2,})?|[ぁ-ゟ]{2,}")
# フォールバック形態素解析時、末尾のひらがな（送り仮名・助詞・動詞尾部）を
# トリムするための正規表現（ISSUE-15）。
KANA_TRAILER = re.compile(r"[ぁ-ゟ]+$")
HEADING = re.compile(r"^(#{1,6})\s+(.*)$")
# 角括弧の構造ラベル・ID（例: `[PREM]`, `[LEM:S4-FIVEELEVEN]`）。全大文字で始まる
# 角括弧トークンを構造マーカーとみなし、プレフィックスをハードコードせず一般に除外する
# （ISSUE-17: 論文の閉包分類ラベル・補題IDが偽の用語として大量検出されていた）。
# 小文字やスペースを含む通常のリンクテキスト `[Some Term](url)` はこの正規表現にマッチ
# しないため、本文語としては従来通り抽出される。
BRACKET_ID = re.compile(r"\[[A-Z][A-Z0-9]*(?::[A-Z0-9][A-Za-z0-9_-]*)?\]")

# 用語抽出前に取り除くスパン（数式・コード・画像・URL・構造ラベルは用語を含まない）。
STRIP = [
